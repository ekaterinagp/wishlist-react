...
export default function Home() {
  ...
  const fetchLists = async (e) => {
    setWishes([]);
    let res = await axios.get("https://wish-back.herokuapp.com/userswishes");
    console.log(res);
  ...
  const fetchUser = async (e) => {
    ...
    if (userId) {
      const res = await axios.get(
        `https://wish-back.herokuapp.com/user/${userId}`
      );
      console.log(res.data);
      if (res.data) {
        setUserData({
  ...
  const fetchFollows = async (e) => {
    ...
    const userId = localStorage.getItem("id");
    const res = await axios.get(
      `https://wish-back.herokuapp.com/followers/${userId}`
    );
  ...
  const addToFollow = async (id) => {
    try {
      ...
      const addedDataRes = await axios
        .post(`https://wish-back.herokuapp.com/follow`, data)
